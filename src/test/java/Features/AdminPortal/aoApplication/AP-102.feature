Feature: AO Application List
  ...
  @Regression @AdminPortal @EBL_MT5 @EIEHK @XPro @AO @EGM
  Scenario: User sees error on the application page if ID number in use exceeded limit
    Given the user logged in to Admin Portal as username "qaauto" and password "Test1234@"
    And the user created up to 4 account with existing ID "qaId"
    ...
    And the user fills id "qaId" on personal information page
