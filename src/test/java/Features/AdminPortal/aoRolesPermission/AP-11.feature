Feature: AO Roles and Permission
  ...
  Scenario: The count of No. of Admin Users is increased after admin user is assigned to the role
    ...
    And the user changes entity role to value "qanoexport" on the user management page
    And the user clicks button "Update" on the user management page
    And the user clicks "Admin Area" on the ao admin portal menu
    And the user clicks "Role & Permission" on the ao admin portal menu
    Then the user sees the number of assigned user of the role "qanoexport" is "increased" on the ao roles and permission page