Feature: Native App AO Application
    ...
    Scenario: User sees an error message when the take profit price of sell order is greater than current price plus BS point
      ...
      And the user fills in the text field "Take Profit" with direction "SELL" and the price is smaller than current price plus BS point on the instrument details page
